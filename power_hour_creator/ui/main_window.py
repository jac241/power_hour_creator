...
from power_hour_creator.ui.creation import (
...
class MainWindow(QMainWindow, Ui_mainWindow):
    def __init__(
        ...
        self._setup_python_to_garbage_collect_after_qt_destroys_cpp_objects()
        self.setupUi(self)
    ...
    def _setup_python_to_garbage_collect_after_qt_destroys_cpp_objects(self):
    ...
    def _setup_power_hour_list_view(self):
        ...
        self.power_hour_model.rowsInserted.connect(
            self.powerHoursListView.select_new_power_hour
        )
    ...
    def _connect_track_errors(self):
        self.tracklist_model.error_downloading.connect(
            self._show_error_downloading
        )

        self.track_error_dispatcher.track_invalid.connect(
            self._show_track_error
        )
    ...
    def _enable_create_power_hour_button_when_tracks_present(self):
        self.tracklist_model.new_power_hour_selected.connect(
            self._try_to_enable_create_button_on_tracklist_change
        )

        self.tracklist_model.dataChanged.connect(
            self._try_to_enable_create_button_on_tracklist_change
        )
    ...
    def _try_to_enable_create_button_on_tracklist_change(self):
        self.createPowerHourButton.setEnabled(
            self.tracklist_model.is_valid_for_export()
        )
    ...
    def _export_power_hour(self):
        power_hour_path = get_power_hour_export_path(
            parent=self, is_video=self._is_video_power_hour()
        )
    ...
    def _connect_file_menu(self):
        ...
        self.action_Export_Current_Tracklist.triggered.connect(
            export_current_tracklist
        )
    ...
    def _connect_power_hour_list_view(self):
        ...
        def show_renamed_power_hour_name(top_left_index, _):
            current_selection = (
                self.powerHoursListView.selectionModel().selectedIndexes()
            )
        ...
        self.powerHoursListView.selectionModel().currentRowChanged.connect(
            show_power_hour_name
        )
        self.powerHoursListView.selectionModel().currentRowChanged.connect(
            show_this_power_hours_tracks
        )
        self.powerHoursListView.selectionModel().currentRowChanged.connect(
            self.tracklist.scrollToTop
        )

        self.powerHoursListView.model().dataChanged.connect(
            show_renamed_power_hour_name
        )
...
def _force_message_box_to_be_wider(msg_box):
    ...
    box_layout.addItem(
        spacer, box_layout.rowCount(), 0, 1, box_layout.columnCount()
    )
